...
const init = async function (options = {}) {
    ...
    allProtocols = [
      function protocols (currentBaseDir) {
        return Hoek.applyToDefaults({
          file: Handlers.file(defaultUserConfigBaseDir),
          path: Handlers.path(defaultUserConfigBaseDir),
          base64: Handlers.base64(),
          env: Handlers.env(),
          require: Handlers.require(defaultUserConfigBaseDir),
          exec: Handlers.exec(defaultUserConfigBaseDir),
          glob: Handlers.glob(defaultUserConfigBaseDir),
          eval: evalHandler
        }, shortstopHandlers)
      },
      ...(protocols ? [protocols] : [])
  ...
  const server = await Steerage.init({
    ...
    userConfigPath: [...(userConfigPath ? [].concat(userConfigPath) : [])]
  })
